...
if not BOT_TOKEN:
    raise ValueError("No token_an found in environment variables!")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"[BOT] Добро пожаловать в анонимный чат для людей, столкнувшихся с булимией и анорексией! Это пространство создано для взаимопомощи и поддержки. Поделись с чатом, а чат поделится с тобой! 😊 \n"
        "Чтобы выйти — /stop.\n\n"
        f"Твой ник: {nickname}\n"
        f"Твой код: {code}\n"
        "Приятного общения!"
